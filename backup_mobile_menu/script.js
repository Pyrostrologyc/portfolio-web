...
// --- Dynamic Content ---
function createPokemonCard(pk) {
    const item = document.createElement('div');
    item.className = 'gallery-item pkball-card';
    ...
    overlay.innerHTML = `<h3>${displayName}</h3>`;
...
function renderGrid(containerId, data) {
    ...
    data.forEach((pk, index) => {
        const item = createPokemonCard(pk);
        // Add interactions to the new card, PASSING THE FULL LIST AND INDEX
        addCardInteractions(item, pk.name, null, data, index);
...
function initDataAndSearch() {
    ...
    if (n3dSearch && typeof n3dData !== 'undefined') {
        n3dSearch.addEventListener('input', (e) => {
            const query = e.target.value.toLowerCase();
            const filtered = n3dData.filter(pk => pk.name.toLowerCase().includes(query));
    ...
    if (cplSearch && typeof cplData !== 'undefined') {
        cplSearch.addEventListener('input', (e) => {
            const query = e.target.value.toLowerCase();
            const filtered = cplData.filter(pk => pk.name.toLowerCase().includes(query));
